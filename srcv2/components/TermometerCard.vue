...
const props = defineProps<{
  items: TermometerItem[];
  meta?: number;
}>();

const getAttributesConfig = (name: string) => {
...
<template>
  <div class="termometer-card">
    <div class="header">
      ...
      <div class="meta-badge" v-if="meta">
        Meta {{ meta }}%
      </div>
    </div>
    ...
    <div class="bars-container">
      <div v-for="item in items" :key="item.id" class="bar-row">
        ...
        <div class="label-name">{{ item.nombre }}</div>
        ...
        <div class="track-wrapper">
          ...
          <div class="track">
            ...
            <div
                ...
                background: getAttributesConfig(item.nombre).gradient
              }"
            >
              <div class="marker">
                <span
                    class="floating-percentage"
                    :style="{ color: getAttributesConfig(item.nombre).color }"
                >
                ...
                <div
                    class="icon-circle"
                    :style="{ backgroundColor: getAttributesConfig(item.nombre).color }"
                >
                  <svg v-if="getAttributesConfig(item.nombre).icon === 'smile'" viewBox="0 0 24 24" fill="currentColor" class="item-icon">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm3.5-9c.83 0 1.5-.67 1.5-1.5S16.33 8 15.5 8 14 8.67 14 9.5s.67 1.5 1.5 1.5zm-7 0c.83 0 1.5-.67 1.5-1.5S9.33 8 8.5 8 7 8.67 7 9.5 7.67 11 8.5 11zm3.5 6.5c2.33 0 4.31-1.46 5.11-3.5H6.89c.8 2.04 2.78 3.5 5.11 3.5z"/>
                  </svg>

                  <svg v-if="getAttributesConfig(item.nombre).icon === 'chat'" viewBox="0 0 24 24" fill="currentColor" class="item-icon">
                    <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/>
                  </svg>

                  <svg v-if="getAttributesConfig(item.nombre).icon === 'heart'" viewBox="0 0 24 24" fill="currentColor" class="item-icon">
                    <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                  </svg>

                  <svg v-if="getAttributesConfig(item.nombre).icon === 'star'" viewBox="0 0 24 24" fill="currentColor" class="item-icon">
